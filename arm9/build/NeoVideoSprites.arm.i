# 1 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoVideoSprites.arm.c"
...
# 2 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoVideoSprites.arm.c" 2
...
# 5 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoVideoSprites.arm.c" 2
# 1 "C:/Users/GRX/NeoDS-GBMacro/arm9/include/NeoMemory.h" 1
# 6 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoVideoSprites.arm.c" 2
...
# 60 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoVideoSprites.arm.c"
static u8* g_spriteCache2Ram;
static TSpriteCacheEntry g_spriteCache[((512*(1<<10)) / ((1 << 3) * 128))] __attribute__((section(".sbss")));
static TSpriteCacheEntry* g_spriteCache2;
static u32 g_spriteCache2Count;
static u32 g_cacheIndex __attribute__((section(".sbss")));
static u32 g_cache2Index __attribute__((section(".sbss")));
static u8* g_spriteReadBuffer;



static u16 g_spriteTable[(64*(1<<20) / ((1 << 3) * 128))] __attribute__((aligned (32)));
static u16* g_spriteTable2;
static u8 g_spriteUsed[(64*(1<<20) / 128) / 8] __attribute__((aligned (32)));

static u8 g_spriteLoadBuffer[(64*(1<<10) / ((1 << 3) * 128)) * ((1 << 3) * 128)] __attribute__((aligned (512)));
static TSpriteTransferEntry g_spriteTransfer[(64*(1<<10) / ((1 << 3) * 128))] __attribute__((aligned (32)));
static u32 g_spriteTransferIndex __attribute__((aligned (32)));
u32 g_spriteCount;

static TSpriteDisplayList* g_spriteList;
# 95 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoVideoSprites.arm.c"
void neoSpriteInit()
{
 s32 i;
 ...
 g_spriteList->cmd = (0x17 << 0) | (0x40 << 8);
 g_spriteList->m[0] = 1 << (12 - 2);
 g_spriteList->m[1] = 0;
 g_spriteList->m[2] = 0;

 g_spriteList->m[3] = 0;
 g_spriteList->m[4] = 1 << (12 - 2);
 g_spriteList->m[5] = 0;

 g_spriteList->m[6] = 0;
 g_spriteList->m[7] = 0;
 g_spriteList->m[8] = 1 << (12 - 2);
 ...
 for(i = 0; i < 32; i++) {
  const s32 y0 = i << (16 + 2);
  const s32 y1 = (i + 1) << (16 + 2);

  g_spriteList->packet[i].cmd0 = (0x2a << 0) | (0x2b << 8) |
   (0x22 << 16);
  g_spriteList->packet[i].cmd1 = (0x25 << 0) | (0x25 << 8) |
   (0x25 << 16) | (0x25 << 24);
 ...
 for(i = 0; i < (64*(1<<20) / ((1 << 3) * 128)); i++) {
  g_spriteTable[i] = 0xff00;
 }
 ...
 for(i = 0; i < ((512*(1<<10)) / ((1 << 3) * 128)); i++) {
  TSpriteCacheEntry* pEntry = &g_spriteCache[i];
  pEntry->index = i;

  neoSystemLoadSprite(g_spriteLoadBuffer, i);

  DC_FlushRange(g_spriteLoadBuffer, ((1 << 3) * 128));
  dmaCopyWords(3, g_spriteLoadBuffer, ((u8*)((uint16*)0x6800000) + i * ((1 << 3) * 128)), ((1 << 3) * 128));
 ...
 memset(g_spriteUsed, 0, (64*(1<<20) / 128) / 8);
 neoSystemLoadRegion(NEOROM_SPRITEUSAGE, g_spriteUsed, (64*(1<<20) / 128) / 8);

  const u32 slot2Free = systemGetSlot2Free();
 u32 cache2Size = 0;

 if(slot2Free >= 20*(1<<20)) {
  cache2Size = 16*(1<<20);
 } else if(slot2Free >= 10*(1<<20)) {
  cache2Size = 8*(1<<20);
 } else if(slot2Free >= 5*(1<<20)) {
  cache2Size = 4*(1<<20);
 }
 ...
 if(cache2Size > 0) {
  ...
  systemSlot2Unlock();
  g_spriteCache2Count = cache2Size / ((1 << 5) * 128);
  g_spriteCache2Ram = systemSlot2Alloc(g_spriteCache2Count * ((1 << 5) * 128));
  g_spriteCache2 = (TSpriteCacheEntry*)systemSlot2Alloc(sizeof(TSpriteCacheEntry) * g_spriteCache2Count);
  g_spriteTable2 = systemSlot2Alloc((64*(1<<20) / ((1 << 5) * 128)) * sizeof(u16));

  for(i = 0; i < (64*(1<<20) / ((1 << 5) * 128)); i++) {
   g_spriteTable2[i] = 0xff00;
  }
  ...
  g_cache2Index = 0;
  g_spriteReadBuffer = systemRamAlloc(((1 << 5) * 128));
  ((void)0);
  systemSlot2Lock();
 } else {
 ...
 (*(vuint32*)0x04000440) = 3;

 (*(vint32*) 0x0400045C) = 16 << (4 + 12 + 12 - 2);
 (*(vint32*) 0x0400045C) = 0;
 (*(vint32*) 0x0400045C) = 0;
...
{
 pPacket->texImage = (tileIndex << 4) |
  ((tileControl & 0x030000) << 2) |
  (1 << 16) |
  (1 << 17) |
  (1 << 20) |
  (1 << 23) |
...
void neoDrawSprites()
{
 ...
 (*(vuint32*)(0x040000B0+(3*12))) = (u32)pDisplayList;
 (*(vuint32*)(0x040000B4+(3*12))) = (u32)&(*(vuint32*) 0x04000400);
 ...
 for(i = 0; i < 381; i++) {
  ...
  if(yControl & 0x40) {
   ...
   fxYPos = (0x200 << 4) - ((s32)(yControl >> 7) << 4);
   zoomY = zoomControl & 0xff;
   fxWidth = ((zoomControl & 0x0f00) + 0x100) >> 8;
   fxHeight = (zoomY + 1) << (4 - 4);

   if(rows > 0x20) {

    rows = 0x20;
    fullmode = 1;
    fxYPosClamp = (zoomY + 1) << (1 + 4);

    if(fxYPos + fxHeight >= (0x100 << 4)) {
     fxYPos -= (0x200 << 4);
    }
    while(fxYPos < (-16) << 4) {
     fxYPos += fxYPosClamp;
  ...
  pDisplayList->m[0] = (fxWidth << (12 - 2));
  pDisplayList->m[4] = (fxHeight << (12 - 2));
  pDisplayList->m[9] = fxXPos;
  pDisplayList->m[10] = fxTileYPos;
  pDisplayList->m[11] = -(1 << 12) + 1 + i;
  ...
  while(fxHeightToDraw > 0) {
   ...
   const u32 cacheEntryIndex = actualIndex >> 3;
   u32 cache1Index = (u32)g_spriteTable[cacheEntryIndex];
   ((void)0);

   if(cache1Index == 0xff00 && spritesUsed < ((512*(1<<10)) / ((1 << 3) * 128)) &&
   g_spriteTransferIndex < (64*(1<<10) / ((1 << 3) * 128))) {

    TSpriteTransferEntry* pTransfer = &g_spriteTransfer[g_spriteTransferIndex++];


    do {
     g_cacheIndex = (g_cacheIndex + 1) & (((512*(1<<10)) / ((1 << 3) * 128)) - 1);
    ...
    g_spriteTable[g_spriteCache[cache1Index].index] = 0xff00;

    g_spriteTable[cacheEntryIndex] = cache1Index;
    g_spriteCache[cache1Index].index = cacheEntryIndex;


    while((*(vuint32*)(0x040000B0+(3*12))) & (1 << (31))) continue;

    if(g_spriteTable2) {
     const u32 cache2EntryIndex = actualIndex >> 5;
     const u32 cache2Offset =
      (actualIndex & ((1 << 5) - 1)) >> 3;
     u32 cache2Index;

     if(lastCache2EntryIndex == cache2EntryIndex) {
      (*(vuint32*)(0x040000B0+(3*12))) = (u32)g_spriteReadBuffer + cache2Offset * ((1 << 3) * 128);
      (*(vuint32*)(0x040000B4+(3*12))) = (u32)pLoadDst;
      (*(vuint32*)(0x040000B8+(3*12))) = ((1 << (31)) | (1 << (26)) | 0) | (((1 << 3) * 128) >> 2);
     } else {

      systemSlot2Unlock();
      cache2Index = g_spriteTable2[cache2EntryIndex];
      if(cache2Index == 0xff00) {
       g_cache2Index++;
       ...
       if(oldCache2EntryIndex != 0xff00) {
        ((void)0);

        g_spriteTable2[oldCache2EntryIndex] = 0xff00;
       }
       ...
       neoSystemLoadSprite2(((void*)((u32)(g_spriteReadBuffer)|0x400000)), cache2EntryIndex);

       systemSlot2Unlock();
       (*(vuint32*)(0x040000B0+(3*12))) = (u32)g_spriteReadBuffer;
       (*(vuint32*)(0x040000B4+(3*12))) = (u32)(g_spriteCache2Ram + cache2Index * ((1 << 5) * 128));
       (*(vuint32*)(0x040000B8+(3*12))) = ((1 << (31)) | (1 << (26)) | 0) | (((1 << 5) * 128) >> 2);
       volatile u32 dummy = (*(vuint32*)(0x040000B8+(3*12)));
       dummy = (*(vuint32*)(0x040000B8+(3*12)));
       while((*(vuint32*)(0x040000B0+(3*12))) & (1 << (31))) continue;
       ...
       (*(vuint32*)(0x040000B0+(3*12))) = (u32)(g_spriteCache2Ram + cache2Index * ((1 << 5) * 128)) + cache2Offset * ((1 << 3) * 128);
       (*(vuint32*)(0x040000B4+(3*12))) = (u32)pLoadDst;
       (*(vuint32*)(0x040000B8+(3*12))) = ((1 << (31)) | (1 << (26)) | 0) | (((1 << 3) * 128) >> 2);
       while((*(vuint32*)(0x040000B0+(3*12))) & (1 << (31))) continue;
    ...
    pLoadDst += ((1 << 3) * 128);
    pTransfer->pDst = ((u8*)((uint16*)0x6800000) + cache1Index * ((1 << 3) * 128));

    (*(vuint32*)(0x040000B0+(3*12))) = (u32)pDisplayList;
    (*(vuint32*)(0x040000B4+(3*12))) = (u32)&(*(vuint32*) 0x04000400);
   }

   if(cache1Index == 0xff00) {
   ...
   ((void)0);
   if(matrixDirty || packetCount >= 32) {

    if(packetCount > 0) {

     while((*(vuint32*)(0x040000B0+(3*12))) & (1 << (31))) continue;
     const u32 listSize = (u32)&pDisplayList->packet[packetCount] - (u32)pDisplayList;
     (*(vuint32*)(0x040000B8+(3*12))) = ((1 << (31)) | (1 << (26)) | (1 << (22)) | (7<<27)) | (listSize >> 2);
    }
    packetCount = 0;
...
skipTile:
 ...
 while((*(vuint32*)(0x040000B0+(3*12))) & (1 << (31))) continue;
 while(((*(vuint32*) 0x04000600) & (1 << (27)))) continue;
 g_spriteCount = (*(uint16*) 0x04000604);
}
...
void neoLoadSprites()
{
 ...
 for(i = g_spriteTransferIndex - 1; i >= 0 ; i--, pTransfer++) {
  (*(vuint32*)(0x040000B0+(3*12))) = (u32)pSrc;
  (*(vuint32*)(0x040000B4+(3*12))) = (u32)pTransfer->pDst;
  (*(vuint32*)(0x040000B8+(3*12))) = ((1 << (31)) | (1 << (26)) | 0) | (((1 << 3) * 128) >> 2);
  pSrc += ((1 << 3) * 128);

  dummy = (*(vuint32*)(0x040000B8+(3*12)));
  dummy = (*(vuint32*)(0x040000B8+(3*12)));
  while((*(vuint32*)(0x040000B8+(3*12))) & (1 << (31))) continue;
 }
