# 1 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c"
...
# 2 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c" 2
...
# 5 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c" 2
# 1 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoProfiler.h" 1
...
# 6 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c" 2
# 1 "C:/Users/GRX/NeoDS-GBMacro/arm9/../common/source/NeoAudioStream.h" 1
# 7 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c" 2




static TNeoAdpcmControl g_adpcmControl[7][32] __attribute__((aligned (32)));



static u16 g_adpcmaBuffer[7][((18432 / 18) * 2)] __attribute__((aligned (32)));
static u16* g_adpcmbBuffer;
static u16 g_audioFrame = 0;
static u32 g_buffer = 0;
static u32 g_zeroFill[7] = {0};
# 28 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c"
...
static const s8 g_adpcmbTable1[16] __attribute__((section(".dtcm"))) = {
 1, 3, 5, 7, 9, 11, 13, 15,
 -1, -3, -5, -7, -9, -11, -13, -15,
};

static const u8 g_adpcmbTable2[16] __attribute__((section(".dtcm"))) = {
...
s16 g_jediTable[49 * 16 * 2] __attribute__((aligned (32)));
# 73 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c"
...
static u8 g_adpcmTable[((16*(1<<20)) / (1 << 10))] __attribute__((aligned (32)));





static u8 g_adpcmCache[250 * (1 << 10)] __attribute__((aligned (32)));
static TADPCMCacheEntry g_adpcmEntry[(250 + 2)] __attribute__((aligned (32)));
...
void neoAudioStreamInit()
{
 ...
 g_adpcmbBuffer = (u16*)neoSystemVramHAlloc((((u32)(55500 / 18 + 4) & ~3) + 4) * 2);

 g_neo->adpcmb.frequency = 18432;
 ...
 for (step = 0; step < 49; step++) {
  ...
  for (nib = 0; nib < 16; nib++) {
   ...
   s32 newStep = (step*16 + g_adpcmStepInc[nib]) * 4;
 ...
 neoSystemLoadRegion(NEOROM_AUDODATA, g_adpcmCache, 250 * (1 << 10));
 for(i = 0; i < ((16*(1<<20)) / (1 << 10)); i++) {
  g_adpcmTable[i] = 0xff;
 }
 for(i = 0; i < 250; i++) {
 ...
 g_adpcmEntry[0].prev = 250;
 g_adpcmEntry[250 - 1].next = (250 +1);

 g_adpcmEntry[250].next = 0;
 g_adpcmEntry[250].prev = 0xff;
 g_adpcmEntry[(250 +1)].next = 0xff;
 g_adpcmEntry[(250 +1)].prev = 250 - 1;
...
void neoAudioStreamReset()
{
 u32 i;
 for(i = 0; i < 7; i++) {
  ((volatile TNeoIPC*)0x027FF400)->pAdpcmBuffer[i] = ((void*)((u32)(g_adpcmaBuffer[i])|0x400000));
  ((volatile TNeoIPC*)0x027FF400)->adpcmControl[i] = ((void*)((u32)(g_adpcmControl[i])|0x400000));
  ((volatile TNeoIPC*)0x027FF400)->adpcmQueuePos7[i] = 0;
  ((volatile TNeoIPC*)0x027FF400)->adpcmQueuePos9[i] = 0;
  ((volatile TNeoIPC*)0x027FF400)->adpcmaFinished[i] = 0;
  memset(g_adpcmaBuffer[i], 0, ((18432 / 18) * 2));
  memset(g_adpcmControl[i], 0, sizeof(TNeoAdpcmControl) * 32);
  g_zeroFill[i] = 0;
 }
 DC_FlushAll();
...
# 196 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c"
static inline void markEntryAsUsed(const u32 cacheIndex)
{
 TADPCMCacheEntry* restrict pEntry = &g_adpcmEntry[cacheIndex];
 TADPCMCacheEntry* restrict pTail = &g_adpcmEntry[(250 +1)];
 ((void)0);
...
{
 const u32 entryIndex = addr >> 10;

 const u32 entryOffset = addr & ((1 << 10) - 1);
 ...
 if(transferSize > (1 << 10) - entryOffset) {
  *length = (1 << 10) - entryOffset;
 }
 return &g_adpcmCache[(entry << 10) + entryOffset];
}
...
static void neoAudioStreamAdpcma(u32 ch, u16* restrict pBuffer, s32 baseStreamSize)
{
 ...
 if((g_neo->adpcmActive & (1 << ch))) {
  const u32 end = pAdpcm->end & ((1 << 20) - 1);
  ...
  while(baseStreamSize > 0) {
   ...
   if((offset & ((1 << 20) - 1)) <= end && (offset & ((1 << 20) - 1)) + streamSize >= end) {
    streamSize = end - (offset & ((1 << 20) - 1));
   }
   ...
   if((offset & ((1 << 20) - 1)) == end) {
    g_neo->adpcmActive &= ~(1 << ch);
    ((volatile TNeoIPC*)0x027FF400)->adpcmaFinished[ch] = 1;
    break;
   }
   ((void)0);
  }
 ...
 } else if(g_zeroFill[ch] < ((18432 / 18) * 2)) {
  g_zeroFill[ch] += baseStreamSize * 2;
  for(i = baseStreamSize; i > 0; i--) {
   *pDst++ = 0;
   *pDst++ = 0;
...
static void neoAudioStreamAdpcmb(u16* restrict pBuffer, const s32 baseStreamSize)
{
 ...
 if((g_neo->adpcmActive & (1 << 6))) {
  ...
  while(bytesToRead > 0) {
   ...
   if((offset & ((1 << 20) - 1)) <= end && (offset & ((1 << 20) - 1)) + bytesRead >= end) {
    bytesRead = end - (offset & ((1 << 20) - 1));
   }
   ...
   if((offset & ((1 << 20) - 1)) == end) {
    ...
    if(loop) {
     ...
     ((volatile TNeoIPC*)0x027FF400)->adpcmaFinished[6] = 1;
 ...
 } else if(g_zeroFill[6] < ((18432 / 18) * 2)) {
  u16* restrict pDst = pBuffer;
...
static void neoAudioCommand(u32 ch, const TNeoAdpcmControl* restrict pControl)
{
 ...
 case NEOADPCM_START:
  ...
  if(pAdpcm->offset < ((volatile TNeoIPC*)0x027FF400)->audioRomSize) {
...
static s32 neoAudioSkipCommands(u32 ch, s32 adpcmQueuePos7, s32 adpcmQueuePos9)
{
 ...
 for(i = 0; i < commandCount; i++) {
  const TNeoAdpcmControl* restrict pControl = &((volatile TNeoIPC*)0x027FF400)->adpcmControl[ch][commandIndex];
...
# 655 "C:/Users/GRX/NeoDS-GBMacro/arm9/source/NeoAudioStream.arm.c"
void neoAudioStreamProcess()
{
 s16 adpcmQueuePos7[7];
 s16 adpcmQueuePos9[7];
 ...
 for(ch = 0; ch < 7; ch++) {
  adpcmQueuePos7[ch] = ((volatile TNeoIPC*)0x027FF400)->adpcmQueuePos7[ch];
  adpcmQueuePos9[ch] = ((volatile TNeoIPC*)0x027FF400)->adpcmQueuePos9[ch];
  ((void)0);
  ((void)0);
  ((void)0);
  ((void)0);
 }
 const u16 arm7StreamCount = ((volatile TNeoIPC*)0x027FF400)->audioStreamCount;
 ...
 if(g_audioFrame != arm7StreamCount) {
  ...
  while((u16)(g_audioFrame + 1) != (u16)arm7StreamCount) {
   ...
   if(g_buffer == 0) g_buffer = (18432 / 18);
   else g_buffer = 0;
   g_audioFrame++;
   ((void)0);
  }
  ...
  for(ch = 0; ch < 7; ch++) {
   ((volatile TNeoIPC*)0x027FF400)->adpcmQueuePos9[ch] = adpcmQueuePos9[ch];
  }
