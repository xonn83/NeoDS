# 1 "C:/Users/GRX/NeoDS-master-old/arm9/source/NeoMemory.c"
...
# 2 "C:/Users/GRX/NeoDS-master-old/arm9/source/NeoMemory.c" 2


# 1 "C:/Users/GRX/NeoDS-master-old/arm9/include/NeoCpu.h" 1
# 5 "C:/Users/GRX/NeoDS-master-old/arm9/source/NeoMemory.c" 2
...
# 6 "C:/Users/GRX/NeoDS-master-old/arm9/source/NeoMemory.c" 2
...
# 8 "C:/Users/GRX/NeoDS-master-old/arm9/source/NeoMemory.c" 2
# 1 "C:/Users/GRX/NeoDS-master-old/arm9/include/NeoMemory.h" 1
# 9 "C:/Users/GRX/NeoDS-master-old/arm9/source/NeoMemory.c" 2
...
static const u8* g_bankTable[(8*(1<<20) / (1 << 12))];
static TCacheOwnerEntry* g_cacheOwner;
static u32 g_bankEntry = 0;
static u32 g_bankCacheCount = 0;


static u8 g_mainProgram[1*(1<<20) + 64*(1<<10)] __attribute__((aligned (32)));
static u8 g_bios[128*(1<<10)] __attribute__((aligned (32)));
static u16 g_vram[0x8000] __attribute__((aligned (32)));
static u16 g_spriteRam[0x800] __attribute__((aligned (32)));
static u16 g_palette[8*(1<<10)] __attribute__((aligned (32)));
static u8 g_sram[64*(1<<10)] __attribute__((aligned (32)));
static u8 g_card[2*(1<<10)] __attribute__((aligned (32)));
static u8* g_audioProgram;

static u8 g_programVector[128] __attribute__((aligned (32)));
...
{
 const u32 loadOffset = 1*(1<<20) + cacheEntryIndex * (1 << 12);
 ((void)0);
 ((void)0);
 u8* pEntry = &g_neo->pRom1[cacheIndex << 12];
 TCacheOwnerEntry* pCacheEntry = &g_cacheOwner[cacheIndex];


 ((void)0);
 g_neo->bankTable[pCacheEntry->index] = ((u8*)0xff000000);
...
const u8* neoLoadBank2(u32 cacheEntryIndex0, u32 cacheIndex0)
{
 ...
 const u32 loadOffset = 1*(1<<20) + cacheEntryIndex0 * (1 << 12);
 ((void)0);
 ((void)0);
 u8* pEntry0 = &g_neo->pRom1[cacheIndex0 << 12];
 u8* pEntry1 = &g_neo->pRom1[cacheIndex1 << 12];
 ...
 ((void)0);
 ((void)0);
 g_neo->bankTable[pCacheEntry0->index] = ((u8*)0xff000000);
 g_neo->bankTable[pCacheEntry1->index] = ((u8*)0xff000000);
...
u32 neoBankPC(u32 a)
{
 ...
 const u32 cacheEntryIndex0 =
  (address >> 12) & ((8*(1<<20) / (1 << 12)) - 1);
 const u8* pEntry0 = g_neo->bankTable[cacheEntryIndex0];
 u32 cacheIndex0 = (u32)(pEntry0 - g_neo->pRom1) >> 12;

 ((void)0);

 if(pEntry0 == ((u8*)0xff000000) || cacheIndex0 == g_bankCacheCount - 1) {
...
void neoMemoryInit()
{
 ...
 g_neo->pBios = g_bios;
 g_neo->pRam = &g_mainProgram[1*(1<<20)];
 g_neo->pSram = g_sram;
 g_neo->pCard = g_card;
 g_neo->pVram = g_vram;
 ...
 for(i = 0; i < (8*(1<<20) / (1 << 12)); i++) {
  g_neo->bankTable[i] = ((u8*)0xff000000);
 }
 ...
 neoSystemLoadRegion(NEOROM_AUDIOPROGRAM, ((void*)((u32)(g_audioProgram)|0x400000)), audioProgramSize);
 ((volatile TNeoIPC*)0x027FF400)->audioProgramSize = audioProgramSize;
 ((volatile TNeoIPC*)0x027FF400)->pAudioProgram0 = ((void*)((u32)(g_audioProgram)|0x400000));
 ((volatile TNeoIPC*)0x027FF400)->audioRomSize = neoSystemRegionSize(NEOROM_AUDODATA);
 ...
 if(g_neo->romBankCount == 0) {
  ...
  u32 bankCount = 1*(1<<20) * g_neo->romBankCount / (1 << 12);
  while(bankCount >= (256*(1<<10) / (1 << 12))) {
   if(systemGetRamFree() >= bankCount * ((1 << 12) + sizeof(TCacheOwnerEntry))) {
    break;
   }
   bankCount >>= 1;
  }
  ...
  g_neo->pRom1 = systemRamAlloc(bankCount * (1 << 12));
  g_cacheOwner = (TCacheOwnerEntry*)systemRamAlloc(bankCount * sizeof(TCacheOwnerEntry));

  for(i = 0; i < g_bankCacheCount; i++) {
   g_bankTable[i] = &g_neo->pRom1[i << 12];
   g_cacheOwner[i].index = i;
  }

  guiConsoleLogf("Loading banks into main ram...");
  neoSystemLoadRegionEx(NEOROM_MAINPROGRAM, g_neo->pRom1, 1*(1<<20), g_bankCacheCount * (1 << 12));

  if(bankCount * (1 << 12) == 1*(1<<20) * g_neo->romBankCount) {
 ...
 neoSystemLoadRegion(NEOROM_MAINPROGRAM, g_neo->pRom0, 1*(1<<20));
 neoSystemLoadRegion(NEOROM_BIOS, g_neo->pBios, 128*(1<<10));
