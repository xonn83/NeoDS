# 1 "C:/Users/GRX/NeoDS/arm9/source/NeoConfig.c"
...
# 2 "C:/Users/GRX/NeoDS/arm9/source/NeoConfig.c" 2
...
# 3 "C:/Users/GRX/NeoDS/arm9/source/NeoConfig.c" 2
...
# 4 "C:/Users/GRX/NeoDS/arm9/source/NeoConfig.c" 2
...
bool neoSaveConfig();
# 5 "C:/Users/GRX/NeoDS/arm9/source/NeoConfig.c" 2
...
# 41 "C:/Users/GRX/NeoDS/arm9/source/NeoConfig.c"
...
static void neoLoadDefaultConfig()
{
 ...
 memset(g_neo->pSram, 0, 64*(1<<10));
 memset(g_neo->pCard, 0, 2*(1<<10));
}
...
bool neoLoadConfig(const char* szFileName)
{
 ...
 if(fileHeader.magic != 0xc07f16f1) {
  ((void)0);
  systemClose(fd);
  return false;
 }
 ...
 for(i = 0; i < fileHeader.chunkCount; i++) {
  ...
  case ((('C') << 24) | (('F') << 16) | (('G') << 8) | ('1')):
   ...
   break;
  case ((('S') << 24) | (('R') << 16) | (('A') << 8) | ('M')):
   if(chunkHeader.size != 64*(1<<10)) {
    ((void)0);
    systemClose(fd);
    return false;
   }
   systemRead(fd, g_neo->pSram, 64*(1<<10));
   break;
  case ((('C') << 24) | (('A') << 16) | (('R') << 8) | ('D')):
   if(chunkHeader.size != 2*(1<<10)) {
    ((void)0);
    systemClose(fd);
    return false;
   }
   systemRead(fd, g_neo->pCard, 2*(1<<10));
   break;
  default:
...
bool neoSaveConfig()
{
 ...
 sramType.type = ((('S') << 24) | (('R') << 16) | (('A') << 8) | ('M'));
 sramType.size = 64*(1<<10);
 systemWrite(fd, &sramType, sizeof(sramType));
 ((void)0);
 guiConsoleDump();
 systemWrite(fd, g_neo->pSram, 64*(1<<10));
 ...
 cardType.type = ((('C') << 24) | (('A') << 16) | (('R') << 8) | ('D'));
 cardType.size = 2*(1<<10);
 systemWrite(fd, &cardType, sizeof(cardType));
 systemWrite(fd, g_neo->pCard, 2*(1<<10));
