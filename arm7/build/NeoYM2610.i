...
static void ymSSGCalculateFreq(const TSSGChannel* pChannel)
{
 ...
 neoAudioBufferWrite16((vu16*)&(*(vu16*)(0x04000408 + ((pChannel->hwChannel)<<4))),
 ...
 if(ymSSGIsNoiseChannel(hwch)) {
  ...
  for(i = 0; i < 3; i++) {
   ...
   if(pChannel->state >= SSG_CHANNEL_WAIT_NOISE) {
    ...
    neoAudioBufferWrite32(&(*(vu32*)(0x04000400 + ((pChannel->hwChannel)<<4))),
     (1 << (31)) | (3<<29) |
  ...
  if(i >= 3) {
   ...
   neoAudioBufferWrite32(&(*(vu32*)(0x04000400 + ((hwch)<<4))), 0, 0xffffffff);
  }
...
static void ymADPCMBCalculateVolume()
{
 ...
 if(pan == 0) {
  ...
  neoAudioBufferWrite32(&(*(vu32*)(0x04000400 + ((6)<<4))), 0, (0x7f|(0x7f << 16)));
  return;
 }
 neoAudioBufferWrite32(&(*(vu32*)(0x04000400 + ((6)<<4))),
...
static void ymADPCMACalculateVolume(s32 ch)
{
 ...
 if(volume >= 63 || pan == 0) {
  ...
  neoAudioBufferWrite32(&(*(vu32*)(0x04000400 + ((ch)<<4))), 0, (0x7f|(0x7f << 16)));
  return;
 }
 ...
 neoAudioBufferWrite32(&(*(vu32*)(0x04000400 + ((ch)<<4))),
