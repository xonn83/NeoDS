# 1 "C:/Users/GRX/NeoDS/arm9/source/NeoVideoFixed.arm.c"
...
# 2 "C:/Users/GRX/NeoDS/arm9/source/NeoVideoFixed.arm.c" 2
...
# 5 "C:/Users/GRX/NeoDS/arm9/source/NeoVideoFixed.arm.c" 2
# 1 "C:/Users/GRX/NeoDS/arm9/include/NeoMemory.h" 1
# 6 "C:/Users/GRX/NeoDS/arm9/source/NeoVideoFixed.arm.c" 2
# 1 "C:/Users/GRX/NeoDS/arm9/include/NeoCpu.h" 1
# 7 "C:/Users/GRX/NeoDS/arm9/source/NeoVideoFixed.arm.c" 2
...
# 21 "C:/Users/GRX/NeoDS/arm9/source/NeoVideoFixed.arm.c"
static TTileCacheEntry g_tileCache[(56*(1<<10) / (4 * 64) - 1)] __attribute__((section(".sbss")));
static u32 g_cacheIndex = 0;

static u16 g_tileTable[((512+128)*2*(1<<10) / (4 * 64))] __attribute__((aligned (32)));
static u8 g_tileUsed[((512+128)*2*(1<<10) / 64) / 8] __attribute__((aligned (32)));


static u8 g_tileLoadBuffer[(4*(1<<10) / (4 * 64)) * (4 * 64)] __attribute__((aligned (32)));
static void* g_tileLoadAddr[(4*(1<<10) / (4 * 64))] __attribute__((aligned (32)));
...
void neoFixedInit()
{
 ...
 (*(vu16*)0x04000050) = (1<<6) | (1<<2) | (1<<11);
 (*(vu16*)0x04000052) = (8 << 0) | (8 << 8);

 g_neo->pTileBuffer = (u16*)neoSystemVramHAlloc(64 * 32 * sizeof(u16));

 for(i = 0; i < ((512+128)*2*(1<<10) / (4 * 64)); i++) {
  g_tileTable[i] = 0xff00;
 }


 for(i = 0; i < (56*(1<<10) / (4 * 64) - 1); i++) {
  TTileCacheEntry* pEntry = &g_tileCache[i];
  pEntry->index = i;
  pEntry->frame = 0xffff;

  neoSystemLoadTile(g_tileLoadBuffer, i);
  memcpy((u8*)((u8*)0x06000000 + (4 * 64)) + i * (4 * 64), g_tileLoadBuffer, (4 * 64));
  g_tileTable[i] = i;
 }

 memset(g_tileUsed, 0, ((512+128)*2*(1<<10) / 64) / 8);
 neoSystemLoadRegion(NEOROM_TILEUSAGE, g_tileUsed, ((512+128)*2*(1<<10) / 64) / 8);
...
void neoDrawFixed()
{
 ...
 const s32 minY = g_videoBounds.minY >> (3 + 4);
 const s32 maxY = (g_videoBounds.maxY + 7) >> (3 + 4);
 const s32 width = g_videoBounds.maxX - g_videoBounds.minX + 1;
 const s32 height = (g_videoBounds.maxY - g_videoBounds.minY + 1) >> 4;
 ...
 g_neo->tileOffsetY = g_videoBounds.minY << (8 - 4);
 ...
 for(mapX = minX; mapX <= maxX; mapX++) {
  ...
  for(mapY = minY; mapY <= maxY; mapY++) {
   ...
   if(g_tileUsed[tileIndex >> 3] & (1 << (tileIndex & 7))) {
    ...
    const u32 cacheEntryIndex = tileIndex / 4;
    ((void)0);
    u32 cacheIndex = g_tileTable[cacheEntryIndex];


    if(cacheIndex == 0xff00 && tilesUsed < (56*(1<<10) / (4 * 64) - 1) &&
    g_tileLoadIndex < (4*(1<<10) / (4 * 64))) {

     void* pLoadAddr = ((void*)((u32)(&g_tileLoadBuffer[g_tileLoadIndex * (4 * 64)])|0x400000));

     do {
      g_cacheIndex++;

      if(g_cacheIndex >= (56*(1<<10) / (4 * 64) - 1)) g_cacheIndex = 0;
     } while(g_tileCache[g_cacheIndex].frame == g_frameCount);
     cacheIndex = g_cacheIndex;
     if(cacheIndex != 0xff00) {


      ((void)0);
      ((void)0);
      g_tileTable[g_tileCache[cacheIndex].index] = 0xff00;
      ...
      g_tileLoadAddr[g_tileLoadIndex] =
       (u8*)((u8*)0x06000000 + (4 * 64)) + cacheIndex * (4 * 64);
      g_tileLoadIndex++;
     }
    }
    if(cacheIndex != 0xff00) {
     ...
     const u32 vramIndex = cacheIndex * 4 +
      (tileIndex & (4 - 1)) + 4;
...
void neoLoadTiles()
{
 ...
 for(i = 0; i < g_tileLoadIndex; i++) {
  dmaCopyWords(3, pSrc, g_tileLoadAddr[i], (4 * 64));
  pSrc += (4 * 64);
 }
